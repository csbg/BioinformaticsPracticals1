...
Use the [`fresh` editor](https://getfresh.dev/) to edit a file. 
...
Now:
- write some text into this file
- save your changes with Ctrl+S
- quit the editor with Ctrl+Q

::: {.callout-tip}
Note that when you save you should see "Saved" written at the bottom of the screen (after the file name, line and column number). If you hit Ctrl+Q before saving, fresh will ask you to discard and quite (type "d" + Enter) or to cancel (type "C" + Enter). It then expects the "d" or "C".
:::

Next, check the content of this file again.

```bash
cat my_file.txt
```

Then, open the file again to edit more text.

```bash
fresh my_file.txt
```

Again, close the file and look at the content.
...
- Copy the file `gRNAs.txt` from your home directory into the directory `day2`.
- Rename the copied file in this directory to `gRNAs_exercise.txt`.
- Add one line at the end of this file (`gRNAs_exercise.txt` that is located in the directory `day2`), adding the gRNA sequence "ACTGACTG". (Note: Make sure to hit enter at the end of the line).
...
- Write the 31th to 35th line of `Homo_sapiens.GRCh38.cds.all.fa.gz` into the file `lines2.txt`. Tip: Extract the first 35 lines and then - of these - the last 5.
